...
fn parseError(filename: []const u8, lineno: u32, comptime fmt: []const u8, args: anytype) noreturn {
    ...
    stderr.interface.print("{s}:{}: parse error: ", .{ filename, lineno }) catch |e| std.debug.panic("write to stderr failed with {s}", .{@errorName(e)});
    stdout.interface.print(fmt, args) catch |e| std.debug.panic("write to stderr failed with {s}", .{@errorName(e)});
